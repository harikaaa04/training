...
class Department:
    ...
    def promote(self, employee: Employee) -> None:
        role = input("Enter the role that the employee has been promoted to: ")
        name, salary = employee.name, employee.salary
        self.employees.remove(employee)
        if role == "Developer":
            employee = Developer(name=name, salary=salary+10000)
        elif role == "Manager":
            employee = Manager(name=name, salary=salary+20000)
        self.employees.append(employee)    
        print(f"{employee.name} has been promoted. His new salary is {employee.salary}")
